...
function gerarPaginaLista(titulo, lista, cor, slug) {
    ...
    return `${headHtml("InvestPop \u2014 " + titulo, titulo + " - FIIs atualizados a cada 10 minutos.", null, "/" + (slug || "") + "/")}
  ...
  <main class="px-4 md:px-8 py-6 md:py-8 max-w-3xl mx-auto">
    ...
    <div class="mb-4">
      <div class="flex items-center bg-card border border-card-border rounded-lg px-3 py-2 gap-2">
        ...
        <input type="text" id="busca" placeholder="Filtrar por nome do FII..." oninput="filtrar()" class="bg-transparent text-base text-gray-300 outline-none w-full" />
      </div>
    </div>
...
function gerarPaginaRanking(titulo, coluna, lista, cor, slug) {
    ...
    return `${headHtml("InvestPop \u2014 " + titulo, titulo + " - Ranking de FIIs atualizado.", null, "/" + (slug || "") + "/")}
