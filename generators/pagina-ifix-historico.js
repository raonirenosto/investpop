...
function gerarPaginaIfixHistorico() {
    ...
    const linhasLista = lista.map((item, i) => `
          <tr class="border-t border-card-border">
            ...
            <td class="py-2 font-medium text-sm"><a href="fiis/${item.cod}.html" class="hover:underline">${item.cod}</a></td>
...
${headerHtml({basePath:'../'})}
  ...
  <main class="px-4 md:px-8 py-6 md:py-8 max-w-5xl mx-auto">
    ...
    <h1 class="text-lg md:text-xl font-bold mb-6">\ud83c\udfe2 Composi\u00e7\u00e3o do IFIX</h1>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-card border border-card-border rounded-xl p-4 md:p-5">
        <h2 class="text-sm font-semibold text-gray-300 uppercase mb-3">Lista Atual (${lista.length} FIIs)</h2>
        <div class="max-h-96 overflow-y-auto">
          <table class="w-full text-sm">
            <thead><tr class="text-gray-500 text-xs"><th class="text-left pb-2">#</th><th class="text-left pb-2">Ticker</th><th class="text-left pb-2">Fundo</th></tr></thead>
...
${historico.length > 0 ? `
        <div class="max-h-96 overflow-y-auto">
          <table class="w-full text-sm">
            <thead><tr class="text-gray-500 text-xs"><th class="text-left pb-2">Data</th><th class="text-left pb-2">Hora</th><th class="text-left pb-2">Ticker</th><th class="text-left pb-2">Fundo</th><th class="text-left pb-2">Tipo</th></tr></thead>
            <tbody class="text-gray-200">
${linhasHistorico}
            </tbody>
          </table>
...
module.exports = { gerarPaginaIfixHistorico }
