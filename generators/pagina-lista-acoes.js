...
function gerarPaginaListaAcoes(titulo, lista, cor) {
    const linhas = lista.map((item, i) => `
          <tr class="border-t border-card-border">
            ...
            <td class="py-2.5 font-medium"><a href="acoes/${item.ticker}.html" class="hover:underline">${item.ticker}</a></td>
  ...
  <main class="px-4 md:px-8 py-6 md:py-8 max-w-3xl mx-auto">
    ...
    <div class="mb-4">
      <div class="flex items-center bg-card border border-card-border rounded-lg px-3 py-2 gap-2">
        ...
        <input type="text" id="busca" placeholder="Filtrar por ticker..." oninput="filtrar()" class="bg-transparent text-base text-gray-300 outline-none w-full" />
      </div>
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="text-gray-500 text-xs">
            ...
            <th class="text-left pb-2 font-medium">A\u00e7\u00e3o</th>
...
${footerHtml(global.INVESTPOP_TESTE ? {teste:true, paginaAcoes:true} : {paginaAcoes:true})}
...
function gerarPaginaRankingAcoes(titulo, coluna, lista, cor) {
    ...
    const tooltips = {
        'DY (12M)': '<div id="tooltip-col" class="tooltip-box hidden"><div class="flex items-center justify-between mb-2"><div class="flex items-center gap-2"><span class="w-6 h-6 rounded-full bg-emerald-500/20 flex items-center justify-center text-[10px] font-bold text-emerald-400">DY</span><span class="font-semibold text-sm text-gray-100">Dividend Yield</span></div><button onclick="closeTooltip()" class="text-gray-500 hover:text-gray-300 text-lg leading-none">&times;</button></div><p class="text-xs text-gray-400 mb-2">Retorno anual em dividendos em rela&ccedil;&atilde;o ao pre&ccedil;o atual da a&ccedil;&atilde;o.</p><p class="text-[10px] text-emerald-400 font-medium mb-1">Exemplo</p><div class="flex items-center gap-3 bg-[#1a2332] rounded-lg p-2"><span class="text-[10px] text-gray-400">Se a a&ccedil;&atilde;o paga R$ 2,00/ano<br>e custa R$ 20,00</span><span class="text-sm font-bold text-emerald-400">DY 10,00%</span></div></div>',
        'P/L': '<div id="tooltip-col" class="tooltip-box hidden"><div class="flex items-center justify-between mb-2"><div class="flex items-center gap-2"><span class="w-6 h-6 rounded-full bg-blue-500/20 flex items-center justify-center text-[10px] font-bold text-blue-400">P/L</span><span class="font-semibold text-sm text-gray-100">Pre&ccedil;o sobre Lucro</span></div><button onclick="closeTooltip()" class="text-gray-500 hover:text-gray-300 text-lg leading-none">&times;</button></div><p class="text-xs text-gray-400 mb-2">Quantos anos de lucro atual seriam necess&aacute;rios para pagar o pre&ccedil;o da a&ccedil;&atilde;o.</p><p class="text-[10px] text-emerald-400 font-medium mb-1">Como interpretar</p><ul class="text-[11px] text-gray-400 space-y-0.5"><li>&bull; P/L baixo &rarr; pode indicar a&ccedil;&atilde;o barata</li><li>&bull; P/L alto &rarr; mercado espera crescimento</li><li>&bull; Comparar com empresas do mesmo setor</li></ul></div>',
        'Var. Ano': '<div id="tooltip-col" class="tooltip-box hidden"><div class="flex items-center justify-between mb-2"><div class="flex items-center gap-2"><span class="w-6 h-6 rounded-full bg-purple-500/20 flex items-center justify-center"><svg class="w-3 h-3 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg></span><span class="font-semibold text-sm text-gray-100">Valoriza&ccedil;&atilde;o no Ano</span></div><button onclick="closeTooltip()" class="text-gray-500 hover:text-gray-300 text-lg leading-none">&times;</button></div><p class="text-xs text-gray-400 mb-2">Varia&ccedil;&atilde;o do pre&ccedil;o da a&ccedil;&atilde;o desde o in&iacute;cio do ano (YTD).</p><p class="text-[10px] text-emerald-400 font-medium mb-1">Como interpretar</p><ul class="text-[11px] text-gray-400 space-y-0.5"><li>&bull; Positivo &rarr; a&ccedil;&atilde;o valorizou no ano</li><li>&bull; N&atilde;o inclui dividendos recebidos</li></ul></div>',
        'Consist\u00eancia': '<div id="tooltip-col" class="tooltip-box hidden"><div class="flex items-center justify-between mb-2"><div class="flex items-center gap-2"><span class="w-6 h-6 rounded-full bg-orange-500/20 flex items-center justify-center"><svg class="w-3 h-3 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg></span><span class="font-semibold text-sm text-gray-100">Consist&ecirc;ncia</span></div><button onclick="closeTooltip()" class="text-gray-500 hover:text-gray-300 text-lg leading-none">&times;</button></div><p class="text-xs text-gray-400 mb-2">Pagamentos consecutivos sem redu&ccedil;&atilde;o no valor dos dividendos/JCP.</p><p class="text-[10px] text-emerald-400 font-medium mb-1">Como interpretar</p><ul class="text-[11px] text-gray-400 space-y-0.5"><li>&bull; Quanto maior, mais previs&iacute;vel o rendimento</li><li>&bull; Tolera picos tempor&aacute;rios</li></ul></div>'
    }
...
module.exports = { gerarPaginaListaAcoes, gerarPaginaRankingAcoes }
